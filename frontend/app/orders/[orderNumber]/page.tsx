    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="mx-auto max-w-4xl px-4 py-4 flex items-center justify-between">
          ...
          <Link href="/orders">
            <Button variant="ghost" size="sm" className="text-gray-500">My Orders</Button>
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <div className="mb-8 text-center">
          <div className="inline-flex items-center justify-center h-16 w-16 rounded-full bg-green-50 text-green-600 mb-4 animate-bounce">
            <CheckCircle2 className="h-10 w-10" />
          </div>
          <h2 className="text-3xl font-black text-gray-900">Thank you for your order!</h2>
          <p className="text-gray-500 mt-2">Order <span className="font-mono font-bold text-blue-600">#{order.orderNumber}</span></p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
            ...
            <Card className="border-none shadow-sm overflow-hidden">
              ...
              <CardContent className="p-0">
                {order.items.map((item, idx) => (
                  <div key={item.id} className={`p-4 flex gap-4 ${idx !== order.items.length - 1 ? 'border-b border-gray-50' : ''}`}>
                    <div className="h-20 w-20 rounded-lg overflow-hidden bg-gray-100 border border-gray-100 flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-black text-blue-600 mt-1">${(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="border-none shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-xs font-bold uppercase tracking-wider text-gray-400">Shipping To</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-sm font-medium text-gray-800">{order.shippingAddress.street}</p>
              ...
              <Card className="border-none shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-xs font-bold uppercase tracking-wider text-gray-400">Status</CardTitle>
                </CardHeader>
                <CardContent>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl border-2 border-dashed border-gray-200 flex items-center gap-3">
              <ShieldCheck className="h-10 w-10 text-blue-600 opacity-50" />
